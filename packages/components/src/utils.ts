...
// ===== 本地工具函数（替代 @antfu/utils）=====

/** 将路径中的反斜杠转换为正斜杠，替代 @antfu/utils 的 slash */
export function slash(str: string): string {
  return str.replace(/\\/g, '/')
}

/** 将值规整为数组，替代 @antfu/utils 的 toArray */
...
/** 过滤 null/undefined 值的类型守卫，替代 @antfu/utils 的 notNullish */
export function notNullish<T>(v: T): v is NonNullable<T> {
  return v != null
}

export interface ThrottleOptions {
  /** 是否禁用首次（leading）调用，默认 false */
  noLeading?: boolean
  /** 是否禁用末尾（trailing）调用，默认 false */
  noTrailing?: boolean
}

/**
 * 节流函数，替代 @antfu/utils 的 throttle。
 *
 * 在指定间隔内最多执行一次：默认首次立即执行（leading），
 * 并在间隔结束后追加一次末尾调用（trailing）以捕获最后一次参数。
 */
export function throttle<T extends (...args: any[]) => any>(
  ...
  const throttled = (...args: any[]) => {
    ...
    // 调度 trailing 调用
    if (!timer && options.noTrailing !== true) {
