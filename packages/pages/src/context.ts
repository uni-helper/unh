 ...
 *
 * **Unh 改造点**：
 * - 持有 `UnhContext` 引用，通过其共享 SFC 解析缓存与文件扫描缓存
 * - `setupWatcher` 改用 `unhCtx.setupWatcher`，全局复用同一 chokidar watcher
 * - `updatePagesJSON` 完成后触发 `pages:extend` hook，供 layouts/manifest 等模块消费
 */
export class PageContext {
   ...
   * @param unhCtx - Unh 共享上下文（可选，未提供时回退到独立模式）
   */
  constructor(userOptions: UserOptions, viteRoot: string = process.cwd(), unhCtx?: UnhContext) {
    this.rawOptions = userOptions
    this.root = slash(viteRoot)
    this.unhCtx = unhCtx || new UnhContext(this.root)
    debug.options('root', this.root)
    this.options = resolveOptions(userOptions, this.root)
    // debug logic
    const debugOption = this.options.debug
    if (debugOption) {
      const prefix = 'unh-pages:'
   ...
   *
   * **Unh 改造点**：通过 `unhCtx` 共享文件扫描缓存（带 chokidar 事件失效）。
   */
  async scanPages() {
    const pageDirFiles = await Promise.all(
      this.options.dirs.map(async dir => ({ dir, files: await getPagePaths(dir, this.options, this.unhCtx) })),
    )
    ...
    for (const path of paths) {
      const page = this.pages.get(path.absolutePath) || new Page(this, path, this.unhCtx)
  ...
  async scanSubPages() {
    ...
    for (const dir of this.options.subPackages) {
      const pagePaths = await getPagePaths(dir, this.options, this.unhCtx)
      paths[dir] = pagePaths

      const pages = new Map<string, Page>()
      for (const path of pagePaths) {
        const page = this.subPages.get(dir)?.get(path.absolutePath) || new Page(this, path, this.unhCtx)
        pages.set(path.absolutePath, page)
      }
   ...
   *
   * **Unh 改造点**：通过 `unhCtx.setupWatcher` 共享全局 chokidar watcher，
   * 避免多个模块各自创建独立 watcher（性能目标：全局 ≤ 1 个 watcher）。
   */
  setupViteServer(server: ViteDevServer) {
    ...
    this._server = server
    // 复用 Vite 自带的 watcher（server.watcher），不额外创建 chokidar 实例
   ...
   *
   * **Unh 改造点**：watcher 由调用方传入（通常是 Vite 的 server.watcher），
   * 本模块不再独立创建 chokidar 实例。缓存失效由 unh-core 的
   * `bindScanInvalidation` / `bindSfcInvalidation` 统一处理。
   *
   ...
   *
   * **Unh 改造点**：生成完成后触发 `pages:extend` hook，
   * 供 layouts / manifest 等模块消费页面路由信息（替代读取 pages.json 文件）。
   *
  ...
  async updatePagesJSON(filepath?: string) {
    ...
    // Unh 改造点：触发 pages:extend hook，通知 layouts/manifest 等模块
    // 路由信息已更新，无需再读取 pages.json 文件中转
    this.unhCtx.hooks.callHook('pages:extend', this.pageMetaData as any)
