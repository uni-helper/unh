...
import { findMacro, parseSFCViaCtx } from './page'
...
/**
 * vite-plugin-uni-pages plugin main entry (兼容导出)
 *
 * 保持与 `@uni-helper/vite-plugin-uni-pages` 相同的 API，
 * 便于现有项目平滑迁移。内部已基于 UnhContext 共享 SFC 解析缓存。
 *
...
export function VitePluginUniPages(userOptions: UserOptions = {}): Plugin {
  ...
  return {
    ...
    async configResolved(config) {
      ...
      if (config.plugins.some(v => v.name === 'vite-plugin-uni-platform' || v.name === 'unh-platform'))
    ...
    async transform(code: string, id: string) {
      ...
      // Unh 改造点：通过 UnhContext 共享 SFC 缓存解析
      const sfc = parseSFCViaCtx(ctx.unhCtx, id, code)
 ...
 * 模块间通过 `pages:extend` hook 通信，禁止直接 import 其他模块。
 */
export const unhPagesModule = defineUnhModule<UserOptions>({
  meta: {
    name: 'unh:pages',
    configKey: 'pages',
  },
  ...
  setup(options, _ctx: UnhContext) {
    const plugin = VitePluginUniPages(options)
    // 将 Vite 插件挂到模块上，由 createUnhVitePlugin 收集
